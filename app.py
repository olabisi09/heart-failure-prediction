...
df = pd.read_csv('heart_failure_clinical_records_dataset.csv') # read the dataset

if app == 'Explore Data':
    about_expander = st.beta_expander('About',expanded=True)
    with about_expander:
        img = Image.open('heartattack.jpg') # import image
        st.image(img)
        st.write("""
    ...
    #displays the dataset according to the user's choice
    ...
    #splits the dataset into the training set and the testing set
    df = pd.read_csv('heart_failure_clinical_records_dataset.csv')
    X = df.drop('DEATH_EVENT', axis=1) #predictor values
    y = df['DEATH_EVENT'] #predicted value
    ...
    if pred_button:    #displays prediction
        if user_result[0] == 0:
            st.success('Patient does not have heart failure (0)')
        else:
            st.error('Patient has heart failure (1)')
